...
public class PlayfairCipher extends AbstractCipher {
	...
	public String decrypt(String[] digrams, String key) {
		...
		for (String ngram : digrams) {
			...
			int col1 = (key.indexOf(ngram.charAt(0)) % 5); // G - 11 % 2 =
															// column 1
			int row2 = (key.indexOf(ngram.charAt(1)) / 5); // C - 2 / 5 = row 0
			int col2 = (key.indexOf(ngram.charAt(1)) % 5); // C - 2 % 5 = column
															// 2
	...
	public String encrypt(String[] digrams, String key) {
		...
		for (String ngram : digrams) {
			int row1 = (key.indexOf(ngram.charAt(0)) / 5);
			int col1 = (key.indexOf(ngram.charAt(0)) % 5);
			
			int row2 = (key.indexOf(ngram.charAt(1)) / 5);
			int col2 = (key.indexOf(ngram.charAt(1)) % 5);
